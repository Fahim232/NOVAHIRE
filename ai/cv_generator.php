    ...
    if (ai_llm_available()) {
        ...
        if ($res['ok'] && !empty($res['text'])) {
            ...
            if (is_array($decoded) && !empty($decoded['full_name'])) {
                ...
                $decoded['target_job'] = $job ? ($job_title . (!empty($company_name) ? ' at ' . $company_name : '')) : null;
...
function _ai_generate_offline_smart_cv($user, $job, $skills_arr, $degree, $exp_raw, $about_raw, $address) {
    ...
    // Summary Synthesis
    $target_str = $job ? ' for the ' . ($job['job_title'] ?? '') . (!empty($job['company_name']) ? ' position at ' . $job['company_name'] : '') : '';
    ...
    return [
        ...
        'target_job'    => $job ? ($job['job_title'] ?? '') . (!empty($job['company_name']) ? ' at ' . $job['company_name'] : '') : null,
        'full_name'     => $username,
        'headline'      => $headline,
        'email'         => $email,
        'phone'         => $phone,
