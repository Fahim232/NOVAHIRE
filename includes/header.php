...
<script>
    function setTheme(themeName) {
        ...
        localStorage.setItem('company-theme', themeName);
    }

    (function() {
        const savedTheme = localStorage.getItem('theme') || localStorage.getItem('company-theme');
        if (savedTheme) {
    ...
    function markNotificationRead(notifId, element) {
        fetch((window.APP_URL || '') + '/api/mark_notification_read.php?id=' + notifId)
            .then(r => r.json())
            .then(data => {
                if (data.success) {
    ...
    function markAllNotificationsRead() {
        fetch((window.APP_URL || '') + '/api/mark_all_read.php')
            .then(r => r.json())
            .then(data => {
                if (data.success) {
    ...
    function showToast(type, title, message, duration = 5000, onclickUrl = '') {
        ...
        if (onclickUrl) toast.style.cursor = 'pointer';
        toast.innerHTML = `
    ...
    setInterval(function() {
        fetch((window.APP_URL || '') + '/api/get_notification_count.php')
            .then(r => {
                if (!r.ok) return null;
                return r.json();
            })
            ...
            .then(function (data) {
                ...
                data.alerts.forEach(function (a) {
                    ...
                    if (typeof showToast === 'function') {
                        showToast('info', a.sender_name, a.message, 7000, (window.APP_URL || '') + '/seeker/live_chat.php?with_company=' + a.sender_id);
                    }
                });
                if (missed > 0) {
